...
mysql.getClient = function(connSettings, cb) {
  var self = this
  connSettings.multipleStatements = true
  var connStr = JSON.stringify(connSettings)
  if (!pools[connStr]) {
    pools[connStr] = MYSQL.createPool(connSettings)
  }
  ...
  pool.getConnection(function(err, connection) {
    ...
    cb(null, connection, connection.release.bind(connection))
  })
}


mysql.end = function(cb) {
...
mysql.determineHosts = function(cb) {
  ...
  async.each(opts.hosts, function(host, callback) {
    ...
    var _port=(_host.length==1?3306:_host[1]); 
    var sql
    var conn = {
      host: _host,
      port: _port ,
