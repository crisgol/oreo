...
firebird.getForeignKeys = function(tableName, cb) {
  ...
  var sql = [
    ...
    ") AS PKTABLE_NAME, TRIM(",
    (_lowercase_keys?' LOWER(ISP.RDB$FIELD_NAME)':'ISP.RDB$FIELD_NAME '),
    
    ") as PKCOLUMN_NAME",
    ",FK.RDB$RELATION_NAME as FKTABLE_NAME, TRIM(",
    ...
    ",PK.RDB$CONSTRAINT_NAME as PK_NAME , TRIM(",
...
firebird.getColumns = function(tableName, cb) {
 ...
 // 'RF.RDB$RELATION_NAME TABLE_NAME,',
  ...
  (_lowercase_keys?' LOWER(RF.RDB$FIELD_NAME)':'RF.RDB$FIELD_NAME'),

 // 'RF.RDB$FIELD_POSITION FIELD_POSITION,',
  ")  AS NAME, IIF(SUBSTRING(RF.RDB$FIELD_SOURCE FROM 1 FOR 4)='RDB$' ,",
...
firebird.upsert = function(client, opts, cb) {
  ...
  if (!opts.pkWhere) //insert
  {
    ...
    var sql = ' select GEN_ID(GEN_'+_tableName+'_ID, '+opts.fbAutoIncrement+') AS ID from rdb$database';
    var _id ='';
     self._execute(client, sql, function(err, rs) {
            ...
            self.execute(client, sql, function(err, rs) {
              ...
              //where com ultimo id
                ...
                var pkField ='ID' ;
                var pkWhere = opts.table + '.' + pkField+ ' = ' + _id
...
firebird.beginTransaction = function(client, callback) {
 ...
 return client.transaction(FIREBIRD.ISOLATION_READ_COMMITED, function(err,transaction){

    //transaction nao funciona com AUTO INCREMENTO
   // if (transaction)                              
   //   client._currentTransaction = transaction;  
    callback(err);
