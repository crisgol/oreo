...
Table.prototype.count = function(opts, cb) {
 ...
 *  - params
 *  - hydrate
 *  - count {Boolean} if true, returns count instead of rows
 */
Table.prototype.find = function(opts, cb) {
  ...
  var select = iq + self.primaryKey.join(iq + ', ' + iq) + iq
  if (opts.count) {
    //select = 'count(*) as count'  
    select = 'count(*) '   // {todo} firebird not accept count , reserved
  }
  var sql = [
    'SELECT ' ,
     '',
    ...
    //'WHERE true'
    'WHERE 1=1 '   // {todo} firebird not accept true 
  ]
  ...
  if (opts) {
    ...
    if (isArray(opts.where)) {
      ...
      Object.keys(opts.where).forEach(function(property) {
        ...
        sql.push('AND ' + iq + property + iq + " = :" + property)
    ...
    if (!opts.count) {
      ...
      // limit
      if (opts.limit) {
        if (self.db._opts.driver ==="node-firebird")  //{TODO} firebird
           //sql.splice ( 1 , 0 ,'FIRST ' + opts.limit)
           sql[1]='FIRST ' + opts.limit
        else
      ...
      if (isInteger(opts.offset)) {
        if (self.db._opts.driver ==="node-firebird")  //{TODO} firebird
           //sql.splice ( 1 , 0 ,'SKIP ' + opts.offset)
           sql[2]='SKIP ' + opts.offset
        else
  ...
  self.db.execute(sql, values, function(err, rs) {
    ...
    if (opts.count) {
      return cb(null, parseInt(rs[0].count, 10))
    }
...
Table.prototype.get = function(key, params, cb) {
  ...
  function getFromDb() {
    ...
    self.db.execute(sql, function(err, rs) {
      ...
      if (!r) {
        var error = new Error('Not found: ' + self.name + ':' + key)
        error.notFound = true
        error.table = self.name
        error.key = key
        return cb(error)
      }
      // save to cache
