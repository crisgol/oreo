...
module.exports = function(params, cb) {
  ...
  let fbAutoIncrement = params.fbAutoIncrement || 1
  var topLevel = false
...
*/
    ...
    function(next) {
      ...
      if (client._nestedTransactionCount === 1) {
         ...
         /*{db._platform.beginTransaction(client, function (err,response)
         
          if (response)                              //TODO firebird       
            client._currentTransaction = response;
          next(err);
         }
         )
*/
    ...
    function(next) {
      ...
      db._platform.upsert(client, {
        ...
        pkWhere: pkWhere,
        data: data,
        fbAutoIncrement : fbAutoIncrement
      }, function(err, row) {
        if (err) {
          err.save = {
            table: table.name,
    ...
    function(next) {
      async.each(Object.keys(data), function (field, done) {
        ...
        async.each(data[field], function (rowData, rowDone) {
          ...
          _fbAutoIncrement++
          foreignRow.save({client: client, 
                          fbAutoIncrement : _fbAutoIncrement}, function (err, savedRow) {
            rowDone(err)
          })
        }, done)
      }, function (err) {
